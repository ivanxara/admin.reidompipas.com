...
export default function Page() {
  ...
  const queryProducts = useQuery({
    queryKey: ['products_menu', MENUS.DAILY.ID],
    queryFn: async () => {
  ...
  const columns = getColumns({
    ...
    refetch: (row, updatedValues) => {
      queryClient.setQueryData(['products_menu', MENUS.DAILY.ID], (oldData: any) =>
        oldData ? oldData.map((item: any) => (item.id === row.original.id ? { ...item, ...updatedValues } : item)) : [],
      );
    },
  });
